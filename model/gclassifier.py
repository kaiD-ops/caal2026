...
class GalaxyClassifierS4D(nn.Module):
    ...
    def forward(self, x, return_logits=False):
        ...
        Architecture Flow
        -----------------
        (B, C, 64, 64) → Hilbert Scan → (B, 4096, C)
                       → Linear Project → (B, 4096, d_model)
                       → S4D + GELU → (B, 4096, d_model)
                       → S4D + GELU → (B, 4096, d_model)
                       → Take Last → (B, d_model)
                       → Linear → (B, num_classes)
                       → (Optional) Softmax → (B, num_classes)
        """
        # Step 1: Hilbert Scan
        # Convert 2D image (B, C, 64, 64) to 1D sequence (B, 4096, C)
        x = self.hilbert_scan(x)  # (B, seq_len, C) where seq_len=4096, C=1 or 3
        
        # Step 2: Input Projection
        # Map pixel values from C channels to d_model dimensions
        x = self.uproject(x)  # (B, 4096, d_model)
        
        # Step 3: First S4D Layer + Activation
        # Process sequence through first S4 block
        x, _ = self.s4_1(x)  # (B, 4096, d_model), S4D returns (output, None)
        x = self.act1(x)     # Apply GELU activation element-wise
        
        # Step 4: Second S4D Layer + Activation
        # Process through second S4 block for deeper representation
        x, _ = self.s4_2(x)  # (B, 4096, d_model)
        x = self.act2(x)     # Apply GELU activation
        
        # Step 5: Extract Final Timestep
        # The last hidden state summarizes the entire sequence
        x = self.take_last(x)  # (B, d_model)
        
        # Step 6: Classification Head
        # Map the sequence summary to class logits
        logits = self.fc(x)  # (B, num_classes)
        
        # Step 7: Optional Softmax
        # Return either raw logits or probabilities
        if return_logits:
            return logits
        else:
