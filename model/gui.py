...
class GalaxyExplorerGUI:
    ...
    def draw(self):
        ...
        # 2. Ensure rgb_render is HxWx3 uint8 RGB image
        # Handle common channel layouts: 2D (H,W), 3D with 1 channel (H,W,1),
        # 3 channels (H,W,3) and 4+ channels (e.g., RGBA). For >=3 channels we take first 3.
        if rgb_render.ndim == 2:
            rgb_render = np.stack([rgb_render, rgb_render, rgb_render], axis=2)
        elif rgb_render.ndim == 3:
            ch = rgb_render.shape[2]
            if ch == 1:
                rgb_render = np.concatenate([rgb_render, rgb_render, rgb_render], axis=2)
            elif ch >= 3:
                rgb_render = rgb_render[:, :, :3]
            else:
                # Unexpected channel count, attempt safe squeeze/repeat
                rgb_render = np.stack([np.squeeze(rgb_render)] * 3, axis=2)

        # Ensure numeric finiteness and proper dtype/range
        # Replace NaN/Inf with 0, then convert/clip to uint8 [0,255]
        if not np.isfinite(rgb_render).all():
            rgb_render = np.nan_to_num(rgb_render, nan=0.0, posinf=255.0, neginf=0.0)

        if rgb_render.dtype != np.uint8:
            try:
                if rgb_render.max() <= 1.0:
                    rgb_render = (rgb_render * 255.0).astype(np.uint8)
                else:
                    rgb_render = np.clip(rgb_render, 0, 255).astype(np.uint8)
            except Exception:
                rgb_render = np.clip(rgb_render.astype(float), 0, 255).astype(np.uint8)

        # Ensure memory is contiguous
        rgb_render = np.ascontiguousarray(rgb_render)

        # Validate final array shape/dtype and provide a safe fallback if invalid
        valid = (isinstance(rgb_render, np.ndarray) and rgb_render.ndim == 3 and rgb_render.shape[2] == 3 and rgb_render.size > 0)
        if not valid:
            try:
                if rgb_render.ndim == 2:
                    h, w = rgb_render.shape
                elif rgb_render.ndim == 3:
                    h, w = rgb_render.shape[0], rgb_render.shape[1]
                else:
                    h = w = self.CANVAS_SIZE
            except Exception:
                h = w = self.CANVAS_SIZE
            print(f"[GalaxyExplorerGUI] Warning: invalid rgb_render array (ndim={getattr(rgb_render,'ndim',None)}, shape={getattr(rgb_render,'shape',None)}, dtype={getattr(rgb_render,'dtype',None)}). Using fallback image.")
            rgb_render = np.zeros((h, w, 3), dtype=np.uint8)

        # Final safety: ensure dtype and contiguity
        if rgb_render.dtype != np.uint8:
            rgb_render = np.clip(rgb_render, 0, 255).astype(np.uint8)
        rgb_render = np.ascontiguousarray(rgb_render)

        # Create surface; if pygame refuses, fall back to a simple filled Surface
        try:
            surface = pygame.surfarray.make_surface(rgb_render.transpose(1, 0, 2))
        except Exception as e:
            print(f"[GalaxyExplorerGUI] pygame.surfarray.make_surface failed: {e}. Falling back to blank surface.")
            surface = pygame.Surface((rgb_render.shape[1], rgb_render.shape[0]))
            surface.fill((0, 0, 0))
